			...
			<td>
			    {!! Form::open(['style' => 'display:inline-block;' ,'method'=> 'PUT', 'route' => ['backend.blog.restore', $post->id ]]) !!}
			        @if(check_user_permissions($request,"Blog@restore", $post->id))
					 ...
					 @else 
						<button title="Restore" onclick="return false;" type="submit" class="btn btn-xs btn-default disabled"}}">
							<i class="fa fa-refresh"></i>
						</button>
					@endif
			 	{!! Form::close() !!}			
			    ...
			    {!! Form::open(['style' => 'display:inline-block;' ,'method'=> 'DELETE', 'route' => ['backend.blog.force-destroy', $post->id ]]) !!}
					...
					@else 
		        		<button title="Delete Permenent" onclick="retun false;" type="submit" class="btn btn-xs btn-danger disabled">  
							<i class="fa fa-times"></i>
		        		</button> 
					@endif
			    {!! Form::close() !!}
